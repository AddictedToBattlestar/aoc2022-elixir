defmodule Day5.Part1SupplyStackBuilder do
  def new(lineData) do
...
defmodule Day5.Part1SupplyStackProcessor do
  ...
  def moveCrates(stackDataTracker, %{numberOfCrates: numberOfCrates, sourceStackNumber: sourceStackNumber, targetStackNumber: targetStackNumber}) do
    ...
    changes = Enum.reduce(1..numberOfCrates, %{cratesRemoved: [], sourceStack: stack}, fn(_, accumulator) ->
      ...
      %{cratesRemoved: [crate | accumulator.cratesRemoved], sourceStack: remainingStack}
    end)
...
defmodule Day5.Part1 do
  def processStackChangesAndFindTopCrates() do
    ...
    stackDataTracker = Day5.Part1SupplyStackBuilder.new(lineData)
    ...
    resultingStackDataTracker = Enum.reduce(messages, stackDataTracker, fn(message, currentStackDataTracker) ->
      IO.puts(message)
      parsedMessage = Day5.Part1SupplyStackProcessor.parseStackChangeMessage(message)
      resultingStackDataTracker = Day5.Part1SupplyStackProcessor.moveCrates(currentStackDataTracker, parsedMessage)
      IO.inspect(resultingStackDataTracker)
      resultingStackDataTracker
    end)

    Day5.Part1SupplyStackProcessor.findTopCratesFromStacks(resultingStackDataTracker)
  end
